            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('anggota.update', $anggota->id) }}" method="POST"
                        ...
                        <!-- NIA -->
                        <div>
                            <label for="nia" class="block text-sm font-medium text-gray-700">NIA (Nomor Induk
                                Anggota)</label>
                            <input type="text" name="nia" id="nia" value="{{ old('nia', $anggota->nia) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Angkatan -->
                            <div>
                                <label for="angkatan" class="block text-sm font-medium text-gray-700">Angkatan <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="angkatan" id="angkatan"
                                    value="{{ old('angkatan', $anggota->angkatan) }}" required
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-red-500 focus:ring-red-500">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <!-- Tipe Anggota -->
                            <div>
                                <label for="tipe_anggota" class="block text-sm font-medium text-gray-700">Tipe Anggota
                                    <span class="text-red-500">*</span></label>
                                <select name="tipe_anggota" id="tipe_anggota" required
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-red-500 focus:ring-red-500">
                                    <option value="Anggota Muda" {{ old('tipe_anggota', $anggota->tipe_anggota) == 'Anggota Muda' ? 'selected' : '' }}>Anggota Muda</option>
                                    <option value="Anggota Biasa" {{ old('tipe_anggota', $anggota->tipe_anggota) == 'Anggota Biasa' ? 'selected' : '' }}>Anggota Biasa</option>
                                    <option value="Anggota Luar Biasa" {{ old('tipe_anggota', $anggota->tipe_anggota) == 'Anggota Luar Biasa' ? 'selected' : '' }}>Anggota Luar Biasa</option>
                                </select>
                        ...
                        <div>
                            ...
                            @if($anggota->foto)
                                <img src="{{ asset('storage/' . $anggota->foto) }}" alt="Foto {{ $anggota->nama }}"
                                    class="h-32 w-32 object-cover rounded-md border border-gray-300 mb-3">
                            @else
                                <p class="text-sm text-gray-500 italic mb-3">Belum ada foto yang diunggah.</p>
                            @endif

                            <label for="foto" class="block text-sm font-medium text-gray-700">Ganti Foto Baru
                                (Opsional)</label>
                            <input type="file" name="foto" id="foto" accept="image/*"
                                class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-red-50 file:text-red-700 hover:file:bg-red-100 border border-gray-300 rounded-md">
                            <p class="mt-1 text-xs text-gray-500">Biarkan kosong jika tidak ingin mengganti foto.
                        ...
                        <div class="flex justify-end pt-4 border-t border-gray-200">
                            <button type="submit"
                                class="bg-red-700 hover:bg-red-800 text-white font-bold py-2 px-6 rounded shadow">
                                Update Data
                            </button>
                        </div>
